...
For a production-shaped rollout, use `compose.production.yaml` with immutable registry images and managed PostgreSQL/Temporal services. It runs migrations as a one-shot dependency, separates API liveness from database readiness, and binds the web service to loopback by default for placement behind a TLS proxy. See [docs/deployment.md](docs/deployment.md) and [.env.production.example](.env.production.example). The API enforces configurable per-process request, guest-session, and mission-mutation limits and dead-letters exhausted dispatch commands. This configuration is deployment scaffolding; the guest-only build still requires production authentication, a shared edge/distributed limiter, secret management, and staging verification before public exposure.
