...
**Current build: durable opportunity analysis with profile evidence, public job snapshots, human approval, mock connector actions, MCP access, measured regression evaluation, and operational telemetry.** New guest workspaces default to a clean real-data profile; the guided synthetic demo is an explicit option. Add and review your own text/PDF/DOCX resume evidence, import a supported HTTPS JobPosting page, inspect live analysis and cited application drafts, revise artifacts, run evaluations, and inspect workspace metrics. A real-data mission requires a named profile with user-ingested evidence, confirmation of its current version, and review of the latest imported job. These real-data checkpoints are implemented locally and await deployment. The in-product Project Guide explains the architecture and safeguards.

Matching and drafts use deterministic rules/templates by default. An optional guarded Agents SDK path can extract source-backed requirements, review explanations, and produce cited drafts while deterministic eligibility, scores, and evidence mappings remain authoritative. Resume ingestion appends evidence, persists provenance-preserving feature-hash embeddings, and applies bounded heuristic parsing while preserving user corrections. Public extraction supports a single JSON-LD JobPosting, including explicitly labeled requirement lists in its description when structured requirement fields are absent. It prefers guarded static HTTPS retrieval and falls back to same-origin Playwright rendering with a stored screenshot. Current public Ashby detail URLs use Ashby's read-only public board API over pinned HTTPS with exact posting-URL matching; removed postings are rejected. When the posting identifies an official company HTTPS URL, a separate checkpoint collects exact description claims and carries their citations into the report and artifacts. Real provider connectors and model-provider evaluations remain pending. The evaluation lab retains the earlier baselines and defaults to a forty-case suite covering matching, weak and partial evidence, weighted scoring, prompt-injection text, eligibility, ambiguity, missing location, and incomplete extraction, with P50/P95 matcher latency. See [PROGRESS.md](PROGRESS.md).
...
An explicitly selected guided demo starts with synthetic fixtures. New real-data guest workspaces start with an empty profile. The profile editor can archive a synthetic profile and start fresh without removing prior missions or documents; archived profiles can be restored. It shows per-field source labels, links inferred fields to supporting resume excerpts, and lets users correct skills, availability, and employment preferences. Real-data missions require a named profile, user-ingested evidence, confirmation of the current profile version, and a reviewed supported job import. Users can remove unsupported extracted requirements, change their importance, and exclude hard constraints during job review; the original source snapshot is retained. Repeat imports indicate whether the saved source snapshot bytes changed. Posting and expiry dates appear when supplied, and expired jobs cannot start real-data missions. Profile changes or a new import require another confirmation.
...
For the personal-project production target, `compose.single-host.yaml` runs the complete stack on one Docker host behind automatic Caddy HTTPS. GitHub Actions verifies every change, publishes immutable API/web images to GHCR, takes a pre-deploy PostgreSQL dump, deploys successful `main` builds over SSH, and checks the public URL. `compose.production.yaml` remains available for a later managed PostgreSQL/Temporal architecture. See [docs/deployment.md](docs/deployment.md), [.env.single-host.example](.env.single-host.example), and [.env.production.example](.env.production.example). The API enforces configurable per-process request, guest-session, and mission-mutation limits and dead-letters exhausted dispatch commands.

## Demo

1. Choose Start guided demo to open a synthetic guest workspace, then choose Northstar.
