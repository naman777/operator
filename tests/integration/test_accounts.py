...
def test_account_claim_login_logout_and_workspace_isolation(tmp_path):
    with TestClient(create_app(f"sqlite:///{tmp_path / 'accounts.db'}")) as client:
        ...
        registered = client.post("/v1/accounts/register", headers=guest_headers, json=credentials)
        ...
        assert account_session["workspace"]["is_demo"] is True
...
def test_account_validation_and_duplicate_email(tmp_path):
    with TestClient(create_app(f"sqlite:///{tmp_path / 'account-validation.db'}")) as client:
        first = client.post("/v1/guest-sessions?demo=true").json()
        ...
        second = client.post("/v1/guest-sessions?demo=true").json()
        second_headers = {"Authorization": f"Bearer {second['token']}"}
        assert (
            client.post("/v1/accounts/register", headers=second_headers, json=credentials).status_code == 409
        )
